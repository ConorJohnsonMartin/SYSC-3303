...
public class Server {
	...
	public void sendAndReceive() throws Exception {
		...
		try {
			...
			printBytes(req);
			System.out.print("\n" + "Server - [REQ] Sent Host Request Packet (String): " + (new String(req)));
		}catch(SocketException se) {
			se.printStackTrace();
			System.exit(1);
		}
		...
		try {
			...
			Thread.sleep(5);
			System.out.print("\n" + "Server - [REC] Received packet (Bytes): ");
			printBytes(packet);
			System.out.println("\n" + "Server - [REC] Received packet (String): " + (new String(packet)));
		}catch(SocketException se) {
			se.printStackTrace();
			System.exit(1);
		}
		...
		try {
			...
			Thread.sleep(5);
			sendSocket.send(sendPacket);
			System.out.print("Server - [SEND] Sending a packet: ");
			printBytes(test);
			System.out.print("\n");
		}catch(SocketException se) {
			se.printStackTrace();
		...
		try {
			...
			printBytes(ack);
			System.out.println("\n" + "Server - [ACK] Packet received by host (String): " + (new String(ack)));
		} catch (IOException e) {
			e.printStackTrace();
			System.exit(1);
		}

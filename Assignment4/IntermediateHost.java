...
public class IntermediateHost {
	...
	public void sendAndReceive() throws IOException, InterruptedException {
		...
		try {
			...
			//Thread.sleep(5);
			System.out.print("Host - [REC] Received Packet from Client containing: " + packet1.length + " bytes" + " Timestamp: " + System.nanoTime());	
		}catch(SocketException se) {
			se.printStackTrace();
			System.exit(1);
		}
		...
		try {
			...
			//System.out.print("\n" + "Host - [ACK] Send Acknowledgement Packet to Client (Bytes): ");
			//printBytes(ack);
			//System.out.print("\n" + "Host - [ACK] Send Acknowledgement Packet to Client (String): " + (new String(ack)) + " Timestamp: " + Instant.now());
		}catch(IOException e) {
			e.printStackTrace();
			System.exit(1);
		}
		...
		try {
			...
			//printBytes(req);
			//System.out.print("\n" + "Host - [REQ] Received Server Request packet (String): " + (new String(req)) + " Timestamp: " + Instant.now());
		}catch(SocketException se) {
			se.printStackTrace();
			System.exit(1);
		}		
		...
		try {
			...
			System.out.print("\n" + "Host - [SEND] Send Packet to Server containing: " + packet1.length + " bytes" + "      Timestamp: " + System.nanoTime());
		}catch(IOException e) {
			e.printStackTrace();
			System.exit(1);
		}
		...
		try {
			...
			//Thread.sleep(5);
			System.out.print("\n" + "Host - [REC] Received Server packet (Bytes): ");
			printBytes(packet2);
		...
		try {
			...
			//printBytes(ack);
			//System.out.print("\n" + "Host - [ACK] Send Acknowledgement Packet to Server (String): " + (new String(ack)) + " Timestamp: " + Instant.now());
		}catch(IOException e) {
			e.printStackTrace();
			System.exit(1);
		}
		...
		try {
			...
			//printBytes(req1);
			//System.out.print("\n" + "Host - [REQ] Received Client Request packet (String): " + (new String(req1)) + " Timestamp: " + Instant.now());		
		}catch(SocketException se) {
			se.printStackTrace();
			System.exit(1);
		}		
		...
		try {
			...
			System.out.print("\n" + "Host - [SEND] Send Packet to Client (Bytes): ");
			printBytes(packet2);
			System.out.print("           Timestamp: " + System.nanoTime());
			System.out.print("\n" + "---------------------------------------------------------------------------------------------------------" + "\n");
		}catch(IOException e) {
	...
	public static void main( String args[]) throws IOException, InterruptedException {
		...
		while(true) {
			...
			host.sendAndReceive();
			//System.out.println("Host Processing: " + System.nanoTime());
		}  
	}	
}

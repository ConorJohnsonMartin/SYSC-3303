...
public class Client {
	...
	public Client() {
		try {
			...
			zero = new byte[] {0};
			wrong = new byte[] {9, 9};
			longMessage = new byte[968];
			//complicated way of calculating the length of byte[] required but I did it just for fun lol.
			readPacket = new byte[read.length + fileName.getBytes().length + zero.length +  mode.getBytes().length + zero.length + longMessage.length];
			writePacket = new byte[write.length + fileName.getBytes().length + zero.length +  mode.getBytes().length + zero.length + longMessage.length];
			invalidPacket = new byte[wrong.length + fileName.getBytes().length + zero.length +  mode.getBytes().length + zero.length + longMessage.length];
	...
	public void sendAndReceive() throws InterruptedException {
		...
		for(int i = 1; i <= 1000; i++) {
			...
			try {
				...
				System.out.println("Client - [SEND] Send Packet containing: " + packet.length + " bytes. Packet #: " + i);
			} catch (IOException e) {
				e.printStackTrace();
				System.exit(1);
			}
			...
			try {
				...
				//Thread.sleep(5);
				//System.out.print("\n" + "Client - [ACK] Packet has been received by host (Bytes): ");
				//printBytes(ack);
				//System.out.println("Client - [ACK] Packet has been received by host (String): " + (new String(ack)) + " Timestamp: " + Instant.now());
			} catch (IOException e) {
				e.printStackTrace();
				System.exit(1);
			}
	...
	public byte[] buildReadPacket(){
		...
		System.arraycopy(longMessage, 0, readPacket, read.length + fileName.getBytes().length + zero.length + mode.getBytes().length + zero.length, longMessage.length);
		return readPacket;
	}
	...
	public byte[] buildWritePacket(){
		...
		System.arraycopy(longMessage, 0, writePacket, write.length + fileName.getBytes().length + zero.length + mode.getBytes().length + zero.length, longMessage.length);
		return writePacket;
	}
	...
	public byte[] buildInvalidPacket(){
		...
		System.arraycopy(longMessage, 0, invalidPacket, wrong.length + fileName.getBytes().length + zero.length + mode.getBytes().length + zero.length, longMessage.length);
